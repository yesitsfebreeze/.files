...
[`corrections`](../prd/00-delivery/corrections/prd.md) if they are not there
...
- macOS only: prepends `/opt/homebrew/{bin,sbin}`, `~/.local/bin` and
  ...
  has no Homebrew — so the bare `nu` in `default_prog` cannot be found and
  the window dies immediately with "No viable candidates found in PATH".
  `env.nu`
  owns `PATH` from inside the shell; this only has to get the binary spawned.
  The same seeding is repeated inline in the two `sh -lc` subprocesses (F6
  theme toggle, `ctrl+shift+b` wallpaper) for the identical reason.
- 2
- 10
----
...
- `ctrl+shift+x` enters from a clean state (clear any stale selection, clear
  the per-pane toggle flag, `ActivateCopyMode`). Inside, the **default**
  `copy_mode` key table is extended rather than replaced — all 55 builtin
  motions and searches survive — with one added binding: plain `c` anchors a
  `Cell` selection on first press and, on second press,
  `CopyTo("ClipboardAndPrimarySelection")` + `CopyMode("Close")`. The toggle
  is tracked per pane id rather than by reading the selection text back,
  because a selection that starts over blank cells reads as empty and would
  desync the toggle; resetting on entry means an exit via `q`/`Esc`/`y` cannot
  leave it stale. Verified in `wezterm show-keys --lua`: `key = 'X', mods =
  'CTRL'` at top level, and `c`/`NONE` inside `key_tables.copy_mode`.
- 4
- 9
----
## Appearance baseline
...
- Flips between the two theme slots `theme.nu` parks in the state dir, via
  ...
  Deferred with tinty itself, pending decision D.1 — the *reader* above is
  minimal base, this *switcher* is not.
- 3
- 6
----
## Per-pane OSC retint  DEFER
- `retint_all_panes` builds an OSC 4/10/11/12/17/19 string from the palette
  ...
  older builds and a pane can die mid-iteration. **Contingent on tinted-shell
  writing per-pane escapes at all** — if D.1 drops tinty, this has no cause.
- 6
- 7
----
...
- Nine terminals, always ready, on **every** window — the startup window,
  ...
  repair so a spawn failure cannot latch the guard and silently disable
  healing for
  the rest of the session; and focus handling that re-asserts the tab you were
