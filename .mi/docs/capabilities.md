...
NOTE: everything that has | in the ## line. descibes what to do with it.
for example we want the docker dev mount to work flawlessy and consolidated as one tool
and everything that has `DO NOT PORT` should not be created/ported at all.
...
- 8
----
## `mount` — drop the current dir into a container
- Shell function that runs `just run "$PWD"` from `~/docker`, building the dev image and dropping into it with the workspace mounted.
- 3
- 7
----
## Nine-tab maximiz:ed startup windows
- `gui-startup`/`gui-attached` hooks spawn a window pre-filled with 9 tabs and maximize it; `Cmd+N` / `Ctrl+Shift+N` spawn more of the same, `Ctrl+Shift+Q` closes every tab at once.
- 3
- 7
----
...
- 8
----
## Neovim: self-bootstrapping config
...
- A one-shot key table where digits 1–9 activate tabs and letters a.. activate panes left-to-right, auto-popping after a single key. Deliberately avoids `PaneSelect` because a Lua-opened modal cannot be dismissed across a tab switch.
- 6
- 8
----
...
- 7
----
## Neovim: shared theme + desktop-style keys
...
- 7
----
## Cross-platform Lua/shell/PowerShell parity
...
- 6
----
## OpenCode theme generation from terminal colors DO NOT PORST
