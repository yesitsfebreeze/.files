...
Parent: [Nushell epic](../prd.md) · C 4 · U 9 · sources: "Zoxide navigation suite"
...
- [ ] **R2** — **`zi`** (and the `cdi` alias) — interactive picker, same
      recents logging.
- [ ] **R3** — **Composed verbs.** `zz` = `cd -` (back-toggle, pairs with the
...
- [ ] **R6** — **Jump only on a genuine match.** Query `zoxide query --exclude
      $PWD` directly (never `__zoxide_z`, whose no-match would cd HOME); no
      match → the normal "command not found" untouched.
