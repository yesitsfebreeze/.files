...
## Requirements
- [ ] **R1** — **`help`** — the overview: the topic list from
      ...
      <topic>`, `help <query>`, `help --all`, the fuzzy browser).
...
- [ ] **R3** — **`help <query>`** — case-insensitive substring/fuzzy match
      across `key`, `cmd`, `title`, and `use`, grouped by topic. `help select`
      must find the shift-select entries.
- [ ] **R4** — **`help <entry>`** — full detail for one entry: title, use,
      why, related (`also`), and its source PRD.
...
- [ ] **R7** — **Non-TTY behavior.** Plain text, no colors, no pager, no TUI
...
- [ ] **R9** — **Container parity.** `help` works inside a capsule; the
      content files ship or mount with it, and entries whose `mode` is
      host-only are marked as such rather than hidden.
