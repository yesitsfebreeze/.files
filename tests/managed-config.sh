...
# not settled here.
#
# `litellm` is the second declared-pending name, added 2026-08-28 by
# 00-delivery/corrections/g1-verify-still-red-on-just-gates R1. It arrived
# undeclared in 0b77a71 (04-shell/10-litellm-launcher, built ahead of its PRD,
# so no spec footprint named this file). It is PENDING and not SURFACE for the
# same reason capsule is: R2 declares nine names, and growing that list is
# 05-platform/01-deploy-mechanism/managed-config's decision, not this gate's.
#
# It is declared at all — rather than moved out from under home/ — because
...
#
# `dot_local` is here deliberately, and it is the one entry that has to argue
# for itself, because check 1 below exists partly to catch a wholesale port of
# the LEGACY source root — whose dot_local is named in that check's own
# comment. This is not that. 0b77a71 deploys five hand-written commands to
# ~/.local/bin (cll, litellm-env, litellm-gen-config, litellm-up, llm-quota),
# and ~/.local/bin is the only PATH-visible target this rebuild has: P.4's
# generator prepends exactly that directory
# (home/run_after_generate-shell-init.sh, `export PATH="$HOME/.local/bin:…"`),
# and home/dot_config/<tool>/ is not on any PATH. So a PATH command has one
# honest place to live and this is it.
#
# The declaration is NARROW on purpose. dot_local names a whole XDG-ish tree —
# share/, state/, lib/ — and admitting the root would admit all of it. Check
# 1b below allows `bin` under dot_local and nothing else, so the wholesale
# port this list is meant to stop still trips.
HOME_TOP=".chezmoiignore .chezmoi.toml.tmpl dot_config dot_gitconfig.tmpl dot_local"
# What may live under dot_local. See the HOME_TOP note above.
DOT_LOCAL_TOP="bin"
...
# dot_local_undeclared <home> — echoes every entry directly under
# <home>/dot_local that DOT_LOCAL_TOP does not name, space-prefixed, or
# nothing. Factored rather than inlined so --selftest can plant against the
# same code the census runs. See the HOME_TOP note at the head of this file.
dot_local_undeclared() {
  local H="$1" e out=""
  [ -d "$H/dot_local" ] || return 0
  while IFS= read -r e; do
    [ -n "$e" ] || continue
    case " $DOT_LOCAL_TOP " in *" $e "*) continue ;; esac
    out="$out $e"
  done < <(find "$H/dot_local" -mindepth 1 -maxdepth 1 -exec basename {} \; | LC_ALL=C sort)
  printf '%s' "$out"
}

# generated_bare <home> — echoes every source file under <home> whose first
# line declares it generated but which does NOT carry chezmoi's create_
# attribute, space-prefixed and relative to <home>, or nothing. The reason
# this matters is in check 7 of stage_surface, where it is measured.
generated_bare() {
  local H="$1" f out=""
  while IFS= read -r f; do
    [ -n "$f" ] || continue
    case "$(basename "$f")" in create_*) continue ;; esac
    out="$out ${f#"$H"/}"
  done < <(grep -rlE '^#+ *GENERATED by ' "$H" 2>/dev/null | LC_ALL=C sort)
  printf '%s' "$out"
}
...
stage_surface() {
  ...
  # 1b. …and dot_local admits `bin` and nothing else. Check 1 declares the
  #     root; without this the declaration would readmit the whole
  #     share/state/lib tree that "a wholesale port of the live source's root"
  #     names. See the DOT_LOCAL_TOP note at the head of this file.
  undeclared="$(dot_local_undeclared "$H")"
  [ -z "$undeclared" ]
  chk "surface: home/dot_local/ holds only declared subtrees (undeclared:${undeclared:- <none>})" $?
  ...
  # 7. A source file that declares ITSELF generated must carry chezmoi's
  #    `create_` attribute. Added 2026-08-28 by
  #    00-delivery/corrections/g1-verify-still-red-on-just-gates R1.
  #
  #    The reason, measured that day rather than reasoned: a plain managed
  #    file is restored on every `chezmoi apply`, and `rr` (04-shell/02 R3) is
  #    `chezmoi update --force`. home/dot_config/litellm/config.yaml shipped
  #    that way in 0b77a71 while `litellm-gen-config` REWRITES the deployed
  #    target at run time (04-shell/10 R9). In a scratch apply the sequence
  #    was: apply -> regenerate -> `chezmoi status` reports
  #    `MM .config/litellm/config.yaml` -> apply -> the regenerated routes are
  #    gone. Renamed to create_config.yaml the same fixture reports a CLEAN
  #    status, keeps the regeneration across an apply, and still re-creates
  #    the file when it is deleted — so a fresh machine still gets a baseline.
  #
  #    The predicate is the file's own first line, not a hand-kept list: the
  #    generator writes the header, so this cannot go stale behind a rename.
  #    `create_` is a chezmoi ATTRIBUTE; the deployed target keeps its name.
  local gen_bad; gen_bad="$(generated_bare "$H")"
  [ -z "$gen_bad" ]
  chk "surface: every self-declared GENERATED file carries chezmoi's create_ attribute (bare:${gen_bad:- <none>})" $?
}
...
stage_selftest() {
  ...
  # RED/GREEN — the dot_local narrowing (check 1b). HOME_TOP now declares
  # dot_local, so the only thing standing between this tree and a wholesale
  # port of the legacy source's ~/.local is DOT_LOCAL_TOP. A declaration
  # nothing can fail is not a declaration, so it is planted against here.
  mkdir -p "$S/home/dot_local/share/nvim"
  echo "      MUTATION: planted home/dot_local/share/ in the copy"
  chk_ok "selftest red: an undeclared home/dot_local/share/ is reported (got:$(dot_local_undeclared "$S/home"))" \
    test -n "$(dot_local_undeclared "$S/home")"
  rm -rf "$S/home/dot_local/share"
  echo "      MUTATION: removed home/dot_local/share/ again (the green counterfactual)"
  chk_ok "selftest green: the unmutated copy reports no undeclared dot_local subtree" \
    test -z "$(dot_local_undeclared "$S/home")"

  # RED/GREEN — the create_ rule (check 7). The mutation is the exact shape
  # 0b77a71 shipped: a self-declared generated file, managed bare.
  printf '# GENERATED by --selftest — regenerate, do not hand-edit.\nk: v\n' \
    > "$S/home/dot_config/litellm/plain.yaml"
  echo "      MUTATION: planted a bare GENERATED home/dot_config/litellm/plain.yaml"
  chk_ok "selftest red: a bare self-declared GENERATED file is reported (got:$(generated_bare "$S/home"))" \
    test -n "$(generated_bare "$S/home")"
  # The same file with only the attribute added must go green — which is what
  # proves the check is about the attribute and not about the filename or the
  # directory.
  mv "$S/home/dot_config/litellm/plain.yaml" "$S/home/dot_config/litellm/create_plain.yaml"
  echo "      MUTATION: renamed it to create_plain.yaml — attribute added, content identical"
  chk_ok "selftest green: the same file with create_ prepended is not reported" \
    test -z "$(generated_bare "$S/home")"

  [ ! -e "$REPO/home/dot_config/foo" ]
  chk "selftest: the real home/dot_config/foo was never created" $?
  [ ! -e "$REPO/home/dot_local/share" ] && [ ! -e "$REPO/home/dot_config/litellm/plain.yaml" ]
  chk "selftest: the real home/dot_local/share and litellm/plain.yaml were never created" $?
}
