...
-- Runs a TUI sidebar in the rightmost pane for interactive session management.
-- When a session is selected, switches the center pane to run that agent.
--
-- The sidebar TUI signals back via OSC 1337 user-var "session_action":
--   select:<id>            — switch center pane to this session
--   rename:<id>:<name>     — rename (already saved by TUI)
...
local TUI_SCRIPT = config_dir .. "/sessions_tui.py"
...
local function send_to_center(tab, session)
  ...
  -- Interrupt current process, clear, then run the agent
  roles.center:send_text("\x03")       -- Ctrl-C
  roles.center:send_text(" clear\r")
  if cmd and #cmd > 0 then
    roles.center:send_text(" " .. cmd .. "\r")
  end
end

-- ── Start sidebar TUI in a pane ─────────────────────────────────

local function start_sidebar_tui(pane)
  local cmd = 'python "' .. TUI_SCRIPT .. '" "' .. SESSIONS_FILE .. '"'
  pane:send_text("\x03")
