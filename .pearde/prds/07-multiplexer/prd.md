...
machine. Governed by
[`tmux-owns-multiplexing-wezterm-keeps-the-chrome`(../../../prds/memos/tmux-owns-multiplexing-wezterm-keeps-the-chrome.md),
...
pane borders print the letters so the label always matches the key. An arrow
in the same switcher splits the view in that direction, the new pane starting
in the active pane's cwd. `F6` toggles the theme, and it works on any terminal that
...
- **An arrow in the switcher splits in that direction, starting in the active
  pane's cwd** — and it deliberately disagrees with the digit's `-c ~`
  (Q14); both rules are driven, not read. F4 no longer exists: its split
  table merged into the switcher on 2026-08-31.
