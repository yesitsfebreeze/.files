...
- [ ] **R1** — The `du -sk` half of the `ls` wrapper (config.nu:114-202) is
...
- [ ] **R2** — The five `$env.config = ($env.config | upsert keybindings …)`
      ...
      unbind.
- [ ] **R3** — Tombstone comments go: config.nu:328-331, :404-418, and every
      line matching `stood here|removed on 20|retired on 20|used to` in the
      footprint. A trap that still bites keeps one line.
- [ ] **R4** — Second-machine branches go: `cf`'s wl-copy/xclip arms
      (config.nu:101-104), `_z_no_zoxide` and its three guards (zoxide.nu:5-7,
      10, 40, 72), theme.nu:162-173, finder.nu:10-15, capsule.nu:236-241.
      `install.sh` installs each tool; a missing one is an install failure,
      not a shell branch.
- [ ] **R5** — `zl`, `zc`, `cdi` (zoxide.nu:51-60) are deleted: the PWD hook
...
- [ ] **R6** — `capsule recent`, `_capsule_record` and the
...
- [ ] **R7** — The cht pipeline (finder.nu:27-41, 57-72, `cht.toml`,
...
- [ ] **R8** — `theme.nu`'s A/B slot files and seed/repair logic (:29-83,
...
- [ ] **R9** — Duplicates collapse: `_finder_shquote` (finder.nu:102-108)
      and `_capsule_shquote` (capsule.nu:219) become one; `_theme_state_dir`
      (theme.nu:8-13) uses `_state_dir` (dirstack.nu:7-12); the
      `XDG_DATA_HOME` default is computed once.
- [ ] **R10** — `env.nu` loses `ENV_CONVERSIONS` for PATH (lines 6-15;
      `$env.PATH | describe` is `list<string>` without it under `nu -n`) and
      the pearde essay (:44-54) becomes the alias plus one line.
- [ ] **R11** — `config.nu:307-321` (re-sourcing tinty's artifact on every
      shell) stays until `05-terminal` proves the tty push covers a new
      shell; this child leaves it and says so in the commit.
- [ ] **R12** — `just manual` is run after the `.nuon` edits.

## Acceptance

- [ ] `nu -l -c 'ls -d | length'` and `nu -l -c 'ls --du | first | get size'` both succeed
- [ ] `nu -l -c '$env.config.keybindings | length'` prints at most 8, and `nu -l -c '$env.PATH | describe'` prints `list<string>`
- [ ] `rg -c 'stood here|removed on 20|retired on 20|used to' home/dot_config/nushell/*.nu` prints nothing
- [ ] `nu -l -c 'zl'`, `nu -l -c 'zc'`, `nu -l -c 'capsule recent'` each fail with "not found"
- [ ] after `chezmoi apply`: `cd` into three directories then `z <bare word>` jumps; F4 enters copy mode; F6 toggles the theme twice and lands back on the first; Ctrl-Q replays a quicklist row; `theme` preview restores the background on Esc
- [ ] `wc -l home/dot_config/nushell/config.nu` prints at most 320
