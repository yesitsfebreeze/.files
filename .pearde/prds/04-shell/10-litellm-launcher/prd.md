...
- [x] **R14** — Context windows are **resolved per route, not assumed**. Every
      route used to carry a flat 200000/64000 (hardcoded in the pi
      `models.json` write), so a 1M model auto-compacted at 200K and a 131K
      local one never did. The generator now resolves a window from the first
      provider in each chain that publishes one — zenmux's `/models` carries
      `context_length` (which is why the zenmux-first order resolves nearly
      everything, measured 2026-08-31: 23 of 27 routes), the opencode store
      carries `contextWindow`, openrouter's `/models` carries
      `context_length`, and ollama's `/api/show` publishes the GGUF trained
      max under `<arch>.context_length` (or `num_ctx` when the Modelfile sets
      it). The resolved numbers land in three places: pi's `models.json`,
      `~/.local/state/litellm/contexts.json` (with the *source* of each number
      recorded, so a fallback can never be read as a measurement), and cll.
      Where nothing publishes, the flat 200000/64000 stays and is labelled
      `default`.
      - The **cll** half is the Claude Code half, and it is constrained by what
        Claude Code documents: it sizes its context against the *model id* and
        has no per-model mapping, so cll declares the picked model's window per
        launch with `CLAUDE_CODE_MAX_CONTEXT_TOKENS`. That variable applies
        directly only to ids Claude Code does not recognise, which is what
        every proxied name is — a `claude-*` alias is left alone on purpose:
        for a real Claude id the variable takes effect only together with
        `DISABLE_COMPACT`, which would disable compaction entirely, and Claude
        Code's own knowledge of the id beats that. A pinned hop looks its
        window up under its base alias (a provider fix is not a different
        model), and the native hop unsets the variable — inherited proxy env
        must not re-size the Max plan's models.
      - The **ollama caveat is on the record**: a local model with no
        explicit `num_ctx` reports its trained max, which is an upper bound —
        the daemon serves at its own default (or `OLLAMA_CONTEXT_LENGTH`)
        and *truncates silently* rather than erroring when a request
        overstays it. The generator prints that warning per affected model
        and the source tag reads `ollama trained` rather than plain
        `ollama`, so an over-windowed claim cannot hide as a measurement.
...
- [x] The regeneration report names a window and a source for every route, and
      no resolved route carries the default: 23 routes `[zenmux]`, 4
      `ollama trained` (gpt-oss-20b and granite4-3b at 131k, the two
      `:cloud` models at 1M, each with the num_ctx warning), 0 `default`.
- [x] cll exports the picked model's window and only that one's: launched with
      a stub `claude` that echoes its environment, `gpt-oss-20b` inherits
      `CLAUDE_CODE_MAX_CONTEXT_TOKENS=131072`, `kimi-k3` 1048576, and a pinned
      `glm-5.2@opencode` inherits glm-5.2's number under its base alias —
      while `claude-opus-5` inherits nothing, per R14.
...
R14 landed 2026-08-31, same mode again ("implement this asap"): the user hit
every model being capped at what turned out to be one hardcoded 200000 in the
pi models.json write. Code and record same session. The env var mechanics
come from the Claude Code docs (model configuration): `CLAUDE_CODE_MAX_CONTEXT_TOKENS`
re-sizes the window only for ids Claude Code does not recognise, and there is
no per-model mapping — hence per-launch export in cll rather than a
settings.json table.
