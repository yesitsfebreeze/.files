...
6. `.pearde/prds/00-delivery/decisions/fzf-model-picker/prd.md` gains one line
   ...
   **DONE ALREADY, by the orchestrator — do not make this edit.** Landed
   2026-09-02 on this spec's own F6: the file belongs to another PRD, and
   every worker brief says never touch another PRD, so the crossing was
   taken by the orchestrator rather than left for a worker to decide. The
   paragraph sits above that node's `## Out of scope` and names
   `~/dev/llm-router/bin/cll`, so this spec's `grep -q 'llm-router'` on it
   is already green and stays green without you. Tick the box below when
   you have read that the line is there; do not rewrite it, and do not add
   a second one. If the grep is red when you run it, stop and report that —
   it means the paragraph was reverted, which is a board problem, not
   yours.
...
- [ ] The `fzf-model-picker` decision node says where the picker now lives — **the orchestrator's box, not the implementer's**; see step 6
