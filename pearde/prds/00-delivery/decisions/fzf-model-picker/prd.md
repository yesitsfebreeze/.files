...
**Where the picker lives, from 2026-09-02.** `cll` — and this fzf picker
with it — leaves this repo for a project of its own at `~/dev/llm-router`,
installed onto PATH from there; the picker is `~/dev/llm-router/bin/cll`.
The user settled it answering `09-simplify/08-litellm-out`'s Q1: *"Its own
project — the router moves to a folder of its own under the development
directory and installs itself onto the path from there."* The decision this
node records is unchanged — fzf is still the picker, and still the second
named exception to I3 — but the code it governs is no longer in this tree,
so a reader looking for the flags this node argues about will not find them
under `home/`. Written by the orchestrator rather than by a worker: the
line crosses out of `09-simplify/08-litellm-out`'s footprint into this
node, and a crossing belongs to a commit, not to a worker's judgment.
