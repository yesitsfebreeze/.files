...
## Requirements
- [x] **R1** — **`cc [...args]`** — Claude with
      `--dangerously-skip-permissions`.
- [x] **R2** — **`cr [...args]`** — same, with `--resume`.
- [x] **R3** — **Profile picker only when it matters.** With a single profile,
...
## Acceptance
- [x] Fresh machine, one login: `cc` starts Claude immediately, no prompt.
- [x] After creating a second profile: `cc` offers the picker, last-used
