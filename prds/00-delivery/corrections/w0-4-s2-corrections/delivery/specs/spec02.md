...
1. **`## Track T`** — delete the first (empty) header row and its `|---|`
   ...
   - `T.6` — [`02-terminal/05`](../../02-terminal/05-tab-content-state/prd.md)
     tab content-state coloring, size `M`, files
     `home/dot_config/wezterm/wezterm.lua`, depends on `T.4`.
   - `T.7` — [`02-terminal/06`](../../02-terminal/06-launchd-path/prd.md)
...
2. **`## Track S`** — add `S.9`,
   [`04-shell/09`](../../04-shell/09-theme-switcher/prd.md) theme switcher
...
3. **`## Track H`** — add `H.1c`,
   [`06-help/01/coverage`](../../06-help/01-content-model/coverage/prd.md),
...
6. **Delete the `Depends on` column from every task table**, header,
   ...
   [`.mi/gantt/plan.json`](../../../gantt/plan.json) and is not restated here,
