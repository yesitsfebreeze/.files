...
- Wrap limit exceeded in several files; tables should be exempted from the
  ...
  lane owned are rewrapped under 80 columns. `[x] fixed` **Re-measured 2026-08-24 and unchanged.** `chezmoi source-path` → `/Users/feb/dev/.files/home`, confirmed by a second probe (`chezmoi data`'s `sourceDir`). `~/.local/share/chezmoi` still exists, HEAD `a2544e4` of 2026-06-20, still a git ancestor of the live `8e99f58` — now two months and four days behind. A census on 2026-08-24 found **13 violating occurrences in 9 files**, 6 of them Tier A. The `AGENTS.md:55-58` carrier is now owned. Two Tier A carriers remain unowned in `docs/capabilities-nushell.md` — line 18 calls the clone "the chezmoi source" outright and line 179 mentions it unlabelled, both in breach of Decision 4(a)'s labelling rule. **This finding stays open until `capabilities-nushell.md` has a node.** New this round, and larger than the original finding: **there are two trees called `.files`.** `~/.files` is the legacy material and is **not a git repository**; `~/dev/.files` is the live chezmoi source, and `AGENTS.md` never named it — so an agent told "legacy lives in `~/.files`" that runs `chezmoi source-path` and sees `/Users/feb/dev/.files/home` has every reason to think it found the legacy tree. `install.sh:256` and `tests/provisioning.sh:23` both cite a `~/.files/install.sh` that does not exist, for the same reason. |
