...
(`](../../04-shell/01-core-config/prd.md)` and `](../01-appearance/prd.md)`)
...
git diff -U0 -- $A $B $C $D | /usr/bin/grep -E '^[-+](state|claim|est|actual|priority|deps|mode|verify|task):'
# expect no output
...
for f in sys.argv[1:]:
    for n, l in enumerate(open(f), 1):
        l = l.rstrip("\n")
        if len(l) > 78 and not l.lstrip().startswith("|") and "](" not in l:
