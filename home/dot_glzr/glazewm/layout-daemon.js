...
// overlay — tell WezTerm what's BEHIND it. WezTerm can't see the z-order, so the
// daemon decides from GlazeWM state and publishes a one-line lua file next to the
// Windows wezterm config: `return true` when a wezterm window is drawn OVER other
// window content (it is fullscreen or floating AND its rect intersects another
// visible window's rect), `return false` when only the wallpaper is behind it.
// A TILED wezterm never overlaps anything — tiles are disjoint (8px gaps) and
// floating windows sit ABOVE tiles — so tiling short-circuits to false. WezTerm
// keeps overlay.lua on its config-reload watch list, so each flip re-applies the
// window opacity within a reload.
...
// Poll cadence: fullscreen toggles (alt+f) emit no subscribed event reliably, so a
// cheap `query windows` every 500ms is the ground truth; events just make it snappy.
...
function onMessage(raw) {
	...
	// Overlay state must track even our own edits (equalize/settle move windows),
	// so check before the self-quiet gate that guards equalize.
	checkOverlay();
...
// ---------------------------------------------------------------------------
// overlay — is any wezterm window drawn over other window content?
// ---------------------------------------------------------------------------
...
// On-screen right now: not minimized, and its workspace is displayed (hidden
// workspaces report displayState 'hidden').
function isVisible(w) {
	if (w.state && w.state.type === "minimized") return false;
	return w.displayState === "shown" || w.displayState === "showing";
}

let overlayChecking = false;
async function checkOverlay() {
	if (overlayChecking) return;
	overlayChecking = true;
	try {
		const data = await send("query windows");
		const visible = ((data && data.windows) || []).filter(isVisible);
		let overlaying = false;
		for (const t of visible) {
			if (t.processName !== "wezterm-gui") continue;
			// Tiled terminal: tiles are disjoint and floats stack above it, so only
			// wallpaper can be behind — never opaque. Fullscreen/floating terminal
			// stacks above whatever its rect covers.
			if (t.state && t.state.type === "tiling") continue;
			if (visible.some((o) => o.id !== t.id && rectsIntersect(t, o))) {
				overlaying = true;
				break;
			}
		}
		if (overlaying === overlayLast) return;
		overlayLast = overlaying;
		fs.mkdirSync(path.dirname(OVERLAY_FILE), { recursive: true });
		// Atomic swap: WezTerm's file watcher can fire mid-write on a plain write.
		const tmp = OVERLAY_FILE + ".tmp";
		fs.writeFileSync(tmp, `return ${overlaying}\n`);
		fs.renameSync(tmp, OVERLAY_FILE);
		log("overlay:", overlaying);
	} catch (err) {
		log("overlay check failed:", err.message);
	} finally {
		overlayChecking = false;
	}
}

setInterval(checkOverlay, OVERLAY_POLL_MS);
...
function command(cmd, subjectId) {
	return send(
		subjectId ? `command --id ${subjectId} ${cmd}` : `command ${cmd}`,
	);
}
...
async function onManaged(id) {
	...
	for (const ms of SETTLE_MS) {
		setTimeout(
			() => settle(id).catch((e) => log("settle error:", e.message)),
			ms,
		);
	}
}
...
async function equalizeNode(node) {
	...
	if (tiling.length > 1) {
		...
		for (let i = 0; i < tiling.length - 1; i++) {
			const size =
				typeof tiling[i].tilingSize === "number"
					? tiling[i].tilingSize
					: target;
