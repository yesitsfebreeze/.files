...
Every window is fullscreen, always: on creation, on every config reload and on every resize, `fill` re-fills a window that is not fullscreen or whose pixel width matches no screen in `wezterm.gui.screens()`. Width only, because fullscreen stops below the MacBook notch (`macos_fullscreen_extend_behind_notch`, default false, WezTerm nightly), so a correct window is shorter than its screen. Extending behind the notch is what made windows jump: on focus macOS pushed the frame down out of the notch, leaving a black strip at the top and pushing tmux's bottom bar off-screen, all at an unchanged size `fill` could not see. The second case is the real one: a display change (resolution switch, screen arrangement) can shove a non-native fullscreen frame half off-screen while WezTerm still reports it fullscreen, and tmux's bottom bar vanishes with it. Toggling off re-fires the resize, which toggles back on against the current screen; a 2 s per-window throttle stops a screen that never matches from toggling forever. `Alt+Enter` is the one way out: it records the window in `wezterm.GLOBAL.windowed` before toggling, and `fill` skips a recorded window, so leaving fullscreen is not undone on the next resize; pressing it again clears the record and fills again. The record is per window id and lives until WezTerm quits. Non-native because macOS's native fullscreen gives each window its own Space: switching windows or monitors then animates a Space change and re-lays the grid mid-slide. Non-native just fills whichever screen the window is on. Moving between the Retina panel (144 dpi) and the QHD monitor (72 dpi) still re-rasterizes the font; that is the dpi change, not the window, and pinning `dpi` would render text at the wrong size on one of them.
