...
# e.g. "base16-mocha" or "base24-espresso". This renders a SELF-CONTAINED,
# representative preview of the scheme — a banner, a small UI mockup, and the
# full palette — by reading the scheme's own hex values and painting them
# with truecolor escapes.
#
...
# macOS ships bash 3.2, which has no associative arrays: `declare -A C` fails
# and `C[0A]` then indexes a plain array, so every subscript is parsed as
# arithmetic and a leading-zero key like 08 dies with "value too great for
# base". Same trap as wezterm-colors.sh. Store one plain variable per key and
# read it back by indirect expansion, which 3.2 does support.
for k in "${keys[@]}"; do eval "C_$k=\"\$(hex \"base\$k\")\""; done
c() { eval "printf '%s' \"\${C_$1:-}\""; }
...
# bold included, and no OSC 11 either. The glyphs carry the layout.
...
# Window background: the terminal is translucent (wezterm
# window_background_opacity), so the card must NOT paint an opaque fill — an
# escape-set bg renders fully opaque and prints a solid rectangle over the
# see-through terminal. Leave WBG empty so every card row inherits the real
# (translucent) terminal background behind it. The selection bar below still
# gets a solid accent fill so the focused row stands out.
WBG=""
...
# row: one card interior line. $1 = colored content (fg only, bg inherits),
# padded to INNER over the window background, framed by single-cell pads and
# borders.
row() { _row "$WBG" "$1"; }
# srow: a full-width selection bar (reverse-fill role) — selection bg, not
# window.
srow() { _row "$SBG" "$1"; }
_row() {
...
swatch() {  # key -> "████ 0D #a78bfa" sized to 15 cells
    ...
    if [ "$color" = 1 ]; then
        # `████` are foreground glyphs: colour them with `fg`, not `bg` (bg
        # paints behind a block that already fills the cell, so the glyph
        # stayed default-fg = white).
